...
public class AOP {
    ...
    @Pointcut("execution(* com.czh.service.StudentService.*(..))")
    //Pointcut表示式
    ...
    @Around(value = "execution(* com.czh.controller.StudentController.*(..))||" +
            "execution(* com.czh.service.StudentService.*(..))")
    //声明该方法是一个环绕通知
    ...
    @AfterReturning(value = "execution(* com.czh.service.StudentService.*(..))", returning = "result")
    //声明该方法是一个返回结果通知
    ...
    @AfterThrowing(value = "execution(* com.czh.service.StudentService.*(..))", throwing = "exp")
    //声明该方法是一个异常通知
